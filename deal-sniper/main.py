...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/deal_sniper.log", encoding="utf-8"),
...
def on_message(message_text: str):
    ...
    for link in links:
        try:
            ...
            # FEATURE 3 — LOGGING IMPROVEMENTS
            logger.info(f"Parsed deal: {parsed_data}")
            logger.debug(f"parsed_data: {parsed_data}")
            logger.debug(f"normalized_url: {parsed_data.get('url')}")
            logger.debug(f"site detection: {parsed_data.get('site')}")

            if not parsed_data or parsed_data["site"] == "unknown":
                continue

            quick_score = score_deal(parsed_data)
            logger.debug(f"quick_score: {quick_score}")
...
async def verification_worker():
    ...
    while True:
        try:
            ...
            # FEATURE 5 — ERROR HANDLING (Watcher request failures must only skip that deal)
            try:
                product_data = await loop.run_in_executor(None, fetch_func, url, session)
            except Exception as e:
                logger.error(f"Watcher fetch failed for {url}: {e}")
                product_data = None
            ...
            # ISSUE 3: Ensure discount detected from "% Off" isn't overwritten when MRP is missing.
            if merged_data['mrp'] is not None and merged_data['mrp'] > 0 and merged_data['price'] is not None:
