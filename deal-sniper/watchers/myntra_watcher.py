...
def fetch_product_data(url: str, session) -> dict:
    """Standard watcher interface for Myntra."""
    def do_fetch():
        rate_limit()
        headers = {
        ...
        # Product Name
        title_elem = soup.find('h1', class_='pdp-title') or soup.find('h1', class_='pdp-name')
        product_name = title_elem.get_text(strip=True) if title_elem else "Unknown Myntra Product"
        ...
        # MRP Check
        mrp = price
        mrp_elem = soup.find('span', class_='pdp-mrp') or soup.find('span', class_='pdp-mrp')
        if mrp_elem:
        ...
        product_id = extract_product_id(url, 'myntra')
