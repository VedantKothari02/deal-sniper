...
def fetch_product_data(url: str, session) -> dict:
    ...
    def do_fetch():
        ...
        # Product Name
        title_elem = soup.find('span', class_='B_NuCI') or soup.find('span', class_='VU-Tz5') or soup.find('span', class_='B_NuCI')
        product_name = title_elem.get_text(strip=True) if title_elem else "Unknown Flipkart Product"

        # Price Check
        price = 0.0
        price_elem = soup.find('div', class_='_30jeq3 _16Jk6d') or soup.find('div', class_='Nx9bqj CxhGGd')
        if price_elem:
        ...
        # MRP Check
        mrp = price
        mrp_elem = soup.find('div', class_='_3I9_wc _2p6lqe') or soup.find('div', class_='yRaY8j A6z5UK')
        if mrp_elem:
